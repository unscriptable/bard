...
define(function (require) {
	...
	function Reactive (rootNode, options) {
		...
		topSection = options.selector('[' + sectionAttr + '="' + options.listName + '"]')[0] || rootNode;
		...
		itemNode = topSection.removeChild(topSection.firstChild).cloneNode(true);
	...
	Reactive.prototype = {
		...
		sortedPos: function (item) {
			...
			return this.binarySearch(
				...
				function (pos) { return compare(item, bindings[pos]); }
			);
		},
		...
		exactPos: function (item) {
			...
			return this.gropeSearch(
				approx,
				function (pos) { return identify(pos) === id; },
				function (pos) { return compare(item, bindings[pos]); }
			);
		},
	 ...
	 * the match function to determine if it has a match.  Uses the evaluate
	 ...
	 * @return {Number}
	 */
	function grope (approx, match, evaluate) {
		var offset = 1, result, tooHigh, tooLow;

		if (match(approx)) return approx;

		do {
			if (!tooHigh) {
				result = match(approx + offset) || evaluate(approx + offset);
